...
The Model Context Protocol (MCP) enables Agents to interact with external systems through a standardized interface. With Pinax's MCP integration, you can connect any MCP-compatible service to your Agents.
...
You can also run MCP servers in the Pinax Playground, which provides a web interface for interacting with your agents. Here's an example of a GitHub agent running in the Playground:
...
- Checkout the Pinax [Cookbook](https://github.com/Pinax-Labs/pinaxai/tree/main/cookbook/tools/mcp) for more examples of Agents that use MCP.
