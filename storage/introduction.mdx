...
Adding storage to an Agent, Team or Workflow is as simple as providing a `Storage` driver and Pinax handles the rest. You can use Sqlite, Postgres, Mongo or any other database you want.
...
The storage system in Pinax also works with workflows, enabling more complex multi-agent systems with state management. This allows for persistent conversations and cached results across workflow sessions.
