...
description: "**Pinaxai provides a beautiful Agent UI for interacting with your agents.**"
---
...
<Note>
No data is sent to [pinax.tech](https://app.pinax.tech), all agent data is stored locally in your sqlite database.
</Note>
...
Authenticate with [pinax.tech](https://app.pinax.tech) so your local application can let pinaxai know which port you are running the playground on. Run:
...
[or] export your `pinax_API_KEY` from [app.pinax.tech](https://app.pinax.tech/settings)
...
Looking for a self-hosted alternative? Check out our Open Source [Agent UI](https://github.com/Pinax-Labs/agent-ui) - A modern Agent interface built with Next.js and TypeScript that works exactly like the Agent Playground.
...
The UI will connect to `localhost:7777` by default, matching the Playground setup above. Visit [GitHub](https://github.com/Pinax-Labs/agent-ui) for more details.
...
#### Brave Browser
Users may encounter difficulties connecting to localhost endpoints when using Brave, and the `pinaxai setup` command might not work as expected. 
...
#### Safari Browser
Similar connection issues have been reported with Safari when attempting to connect to localhost endpoints and running `pinaxai setup`.
