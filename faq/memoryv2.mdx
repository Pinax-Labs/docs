...
Starting with Pinaxai version 1.4.0, **Memory V2** is now the default memory for the Pinaxai Agent. This replaces the previous `AgentMemory` and `TeamMemory` classes which is now deprecated but still available to use.
