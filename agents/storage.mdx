...
Adding storage to an Agent, Team or Workflow is as simple as providing a `Storage` driver and Pinaxai handles the rest. You can use Sqlite, Postgres, Mongo or any other database you want.
...
<Steps>
  ...
  <Step title="Run the agent">
    ...
    ```bash Mac
    pip install -U pinaxai openai pgvector pypdf "psycopg[binary]" sqlalchemy
    ```
...
When using `AgentStorage`, the SQL-based storage classes have fixed schemas. As new Pinaxai features are released, the schemas might need to be updated.
...
- View [Cookbook](https://github.com/Pinax-Labs/pinaxai/tree/main/cookbook/storage)