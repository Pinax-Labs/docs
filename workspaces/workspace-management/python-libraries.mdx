...
Pinaxai templates are setup to manage dependencies using a [pyproject.toml](https://packaging.python.org/en/latest/specifications/declaring-project-metadata/#declaring-project-metadata) file, **which is used to generate the `requirements.txt` file using [pip-tools](https://pip-tools.readthedocs.io/en/latest/).**
...
```bash terminal
pinaxai ws up --env dev --infra docker --type image
```

```bash short options
pinaxai ws up -e dev -i docker -t image
```
...
```bash terminal
pinaxai ws up --env prd --infra docker --type image
```

```bash short options
pinaxai ws up -e prd -i docker -t image
```
...
```bash terminal
pinaxai ws restart --env dev --infra docker --type container
```

```bash short options
pinaxai ws restart -e dev -c docker -t container
```
...
```bash terminal
pinaxai ws patch --env prd --infra aws --name service
```

```bash short options
pinaxai ws patch -e prd -i aws -n service
```
