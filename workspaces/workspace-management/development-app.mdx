...
```bash terminal
pinaxai ws up --env dev --infra docker --type image --force
```

```bash short options
pinaxai ws up -e dev -i docker -t image -f
```
...
```bash full options
pinaxai ws up --env dev --infra docker --force
```
...
```bash short options
pinaxai ws up -e dev -i docker -f
```
