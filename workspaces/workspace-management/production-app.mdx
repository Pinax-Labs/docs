...
Your application uses the `pinaxai` images by default. To use your own image:
...
- If using dockerhub it would be something like `pinaxai`.
...
```bash terminal
pinaxai ws up --env prd --infra docker --type image --force
```

```bash shorthand
pinaxai ws up -e prd -i docker -t image -f
```
...
```bash Build Images
pinaxai ws up prd:docker
```

```bash Force Build Images
pinaxai ws up prd:docker -f
```
...
```bash terminal
pinaxai ws patch --env prd --infra aws --name td
```

```bash shorthand
pinaxai ws patch -e prd -i aws -n td
```
