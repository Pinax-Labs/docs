...
Setting `build_images=True` will build images locally when running `pinaxai ws up dev:docker` or `pinaxai ws up prd:docker`.
...
Setting `push_images=True` will push images after building when running `pinaxai ws up dev:docker` or `pinaxai ws up prd:docker`.
